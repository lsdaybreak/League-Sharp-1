...
namespace HaxDeTryndamere
{
    class Program
    {
        ...
        private static void Bloodlust()
        {
            ...
            {
                if (ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsEnemy && x.Distance(Player.Position) <= 1100).Count() > 0)
                {
                    Q.Cast();
                }
            }
        ...
        {
            if (R.IsReady() && ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsEnemy && x.Distance(Player.Position) <= 1100).Count() > 0 && (Player.Health / Player.MaxHealth) * 100 < Menu.Item("rBelow").GetValue<Slider>().Value)
            {
                R.Cast();
            }
        }
